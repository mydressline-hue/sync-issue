...
export async function performCombineImport(
  ...
  // Step 1: Clean data
  console.log(
    `[performCombineImport] Step 1: Cleaning ${allItems.length} items...`,
  );
  const cleanResult = await cleanInventoryData(allItems, dataSource.name);
  console.log(
    `[performCombineImport] After clean: ${cleanResult.items.length} items remain`,
  );
  ...
  console.log(
    `[performCombineImport] After import rules: ${importRulesResult.items.length} items remain`,
  );
  ...
  console.log(
    `[performCombineImport] After variant rules: ${ruleResult.items.length} items remain`,
  );
  ...
  if (
    ...
    if (shopifyStoreId) {
      console.log(
        `[Combine] Applying price-based size expansion for "${dataSource.name}"`,
      );
      try {
        const cacheVariants =
          await storage.getVariantCacheProductStyles(shopifyStoreId);
  ...
  if (!isSaleFile && linkedSaleDataSourceId) {
    console.log(
      `[Combine] Regular file "${dataSource.name}" - checking for discontinued styles`,
    );
    ...
    if (discontinuedItemsRemoved > 0) {
      console.log(
        `[Combine] Removed ${discontinuedItemsRemoved} existing items with discontinued styles`,
      );
    }
  ...
  const dedupOffsetCombine =
    (dataSource as any).stockInfoConfig?.dateOffsetDays ?? 0;
  const dedupResultCombine = deduplicateAndZeroFutureStock(
    itemsToImport,
    dedupOffsetCombine,
  );
  ...
  console.log(
    `[performCombineImport] Step 8: Saving ${itemsToImport.length} items to database (strategy=${updateStrategy})`,
  );
  let importedCount = 0;
  let addedCount = 0;
  let updatedCount = 0;
  ...
  if (itemsToImport.length > 0) {
    if (updateStrategy === "full_sync") {
      ...
      console.log(
        `[Combine Upsert] ${dataSource.name}: Upserting ${itemsToImport.length} items`,
      );
      const isRegularInventory = (dataSource as any).sourceType !== "sales";
      const result = await storage.upsertInventoryItems(
        itemsToImport,
    ...
    console.log(
      `[Combine Full Sync] ${dataSource.name}: No items to import and no existing items`,
    );
  }
  ...
  console.log(
    `[performCombineImport] DONE: importedCount=${importedCount}, strategy=${updateStrategy}, filesProcessed=${stagedFiles.length}`,
  );
  return {
    success: true,
...
export async function registerRoutes(
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const dedupOffset =
          (dataSource as any).stockInfoConfig?.dateOffsetDays ?? 0;
        const dedupResult = deduplicateAndZeroFutureStock(
          step05FilteredItems,
          dedupOffset,
        );
  ...
  app.post("/api/data-sources/:id/fetch-url", async (req, res) => {
    try {
      ...
      // ============================================================
      // Dedup by style-color-size and zero out stock for future ship dates
      const dedupOffsetUrl =
        (dataSource as any).stockInfoConfig?.dateOffsetDays ?? 0;
      const dedupResultUrl = deduplicateAndZeroFutureStock(
        itemsAfterExpansion,
        dedupOffsetUrl,
      );
