...
export async function processUrlDataSourceImport(
  ...
  try {
    ...
    if (saleImportCheck.requiresWarning) {
      console.log(
        `[URL Import] SKIPPED: ${dataSource.name} - ${saleImportCheck.warningMessage}`,
      );
      return {
        success: false,
        error: `Sale file not imported: ${saleImportCheck.warningMessage}`,
      };
    }
    ...
    if (validationConfig.enabled !== false) {
      console.log(
        `[URL Import] Validating file "${filename}" for data source "${dataSource.name}"`,
      );
      ...
      if (!validation.valid) {
        ...
        console.error(
          `[URL Import] SAFETY BLOCK: File "${filename}" failed validation:`,
          validation.errors,
        );
        return {
          success: false,
          error: `SAFETY NET: Import blocked - ${validation.errors.join("; ")}`,
        };
      }

      if (validation.warnings.length > 0) {
        console.log(`[URL Import] Validation warnings:`, validation.warnings);
      }
    }
    ...
    if (items.length === 0) {
      return { success: false, error: "File contains no valid data rows" };
    }
    ...
    // Helper to convert color to Title Case
    ...
    if (
      ...
      if (shopifyStoreId) {
        console.log(
          `[URL Import] Applying price-based size expansion for "${dataSource.name}"`,
        );
        try {
          const cacheVariants =
          ...
          if (priceBasedExpansionCount > 0) {
            console.log(
              `[URL Import] Price-based expansion added ${priceBasedExpansionCount} size variants`,
            );
          }
        } catch (expansionError) {
          console.error(
            `[URL Import] Price-based expansion error:`,
            expansionError,
          );
        }
      }
    ...
    if (shopifyStoreIdForCompareAt && useCompareAtPrice && isSaleFile) {
      try {
        const skus = itemsAfterExpansion
          .map((item: any) => item.sku)
          .filter((sku: string | null) => sku && sku.trim());

        if (skus.length > 0) {
          console.log(
            `[URL Import] Looking up Shopify prices for ${skus.length} SKUs...`,
          );
        ...
        console.error("[URL Import] Error loading Shopify prices:", err);
      }
    }
    ...
    const stockInfoRule = await getStockInfoRule(dataSource, storage);
    if (stockInfoRule) {
      console.log(
        `[URL Import] Calculating stockInfo for ${itemsAfterExpansion.length} items`,
      );
      itemsAfterExpansion = itemsAfterExpansion.map((item: any) => ({
        ...item,
    ...
    // Import based on update strategy
    ...
    if (itemsAfterExpansion.length > 0) {
      if (updateStrategy === "full_sync") {
        ...
        console.log(
          `[URL Import Full Sync] ${dataSource.name}: Atomic replace complete - deleted ${result.deleted}, created ${result.created} items`,
        );
      } else {
        console.log(
          `[URL Import Upsert] ${dataSource.name}: Upserting ${itemsAfterExpansion.length} items`,
        );
      ...
      if (existingCount > 0) {
        console.error(
          `[URL Import] SAFETY BLOCK: File has 0 items but data source "${dataSource.name}" has ${existingCount} existing items.`,
        );
        return {
          success: false,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // Single file mode - import immediately
        // Check update strategy
        const updateStrategy =
          (dataSource as any).updateStrategy || "full_sync";
        ...
        const getStylePrefix = (style: string): string => {
          ...
          // If custom prefixes are enabled and rules exist
          if (
            cleaningConfig?.useCustomPrefixes &&
        ...
        if (filteredItems.length < items.length) {
          console.log(
            `[Manual Upload] Filtered out ${items.length - filteredItems.length} items based on skip rule`,
          );
        }

        // Step 0.5: Filter out discontinued items that have ZERO stock (matching email import behavior)
        // "D" means discontinued - but if there's still stock, we should import it to sell remaining inventory
        // Only filter out: discontinued === true AND stock === 0 AND no future stock flags
        // This removes truly dead inventory while preserving pre-orders
        let step05FilteredItems = filteredItems;
        ...
        if (discontinuedZeroStock.length > 0) {
          const beforeFilter = step05FilteredItems.length;
          step05FilteredItems = step05FilteredItems.filter((item: any) => {
          ...
          const actuallyFiltered = beforeFilter - step05FilteredItems.length;
          if (actuallyFiltered > 0) {
            console.log(
              `[Manual Upload] Step 0.5: Filtered out ${actuallyFiltered} discontinued (D) items with zero stock (preserved ${discontinuedZeroStock.length - actuallyFiltered} with future stock)`,
            );
          }
        }
        ...
        const inventoryItems = step05FilteredItems.map((item: any) => {
          ...
          const prefixedSku =
            ...
            item.size !== ""
              ? `${prefixedStyle}-${normalizedColor}-${item.size}`
                  .replace(/\//g, "-") // Convert slashes to hyphens in SKU
                  .replace(/\s+/g, "-")
                  .replace(/-+/g, "-")
        ...
        if (
          ...
          if (shopifyStoreId) {
            console.log(
              `[Import] Applying price-based size expansion for data source "${dataSource.name}"`,
            );
            try {
              ...
              const stylePriceMap = buildStylePriceMapFromCache(cacheVariants);
              console.log(
                `[Import] Built style price map with ${stylePriceMap.size} styles`,
              );
              ...
              if (priceBasedExpansionCount > 0) {
                console.log(
                  `[Import] Price-based expansion added ${priceBasedExpansionCount} size variants`,
                );
              }
            } catch (expansionError) {
              console.error(
                `[Import] Price-based expansion error:`,
                expansionError,
              );
            ...
            console.log(
              `[Import] Price-based expansion enabled but no Shopify store linked - skipping`,
            );
          }
        }

        // Step 3: Handle discontinued styles based on source type
        const isSaleFile = (dataSource as any).sourceType === "sales";
        const linkedSaleDataSourceId = (dataSource as any)
          .assignedSaleDataSourceId;
        let discontinuedStylesFiltered = 0;
        let discontinuedItemsRemoved = 0;

        // Get the final items to import (use expanded items if price-based expansion was applied)
        let itemsToImport = itemsAfterExpansion;

        if (!isSaleFile && linkedSaleDataSourceId) {
          // Regular file with linked sale file: Filter out discontinued styles and remove existing items
          console.log(
            `[Import] Regular file "${dataSource.name}" - checking for discontinued styles from sale file`,
          );
          ...
          if (discontinuedItemsRemoved > 0) {
            console.log(
              `[Import] Removed ${discontinuedItemsRemoved} existing inventory items with discontinued styles`,
            );
          }
          ...
          if (discontinuedStylesFiltered > 0) {
            console.log(
              `[Import] Filtered out ${discontinuedStylesFiltered} items with ${filterResult.discontinuedStyles.length} discontinued styles: ${filterResult.discontinuedStyles.slice(0, 3).join(", ")}${filterResult.discontinuedStyles.length > 3 ? "..." : ""}`,
            );
          }
        }
        ...
        if (shopifyStoreId && useCompareAtPrice && isSaleFile) {
          try {
            ...
            if (skus.length > 0) {
              console.log(
                `[Import] Looking up Shopify prices for ${skus.length} SKUs (sale file compare-at)...`,
              );
              const cachedVariants = await storage.getVariantCacheBySKUs(
                shopifyStoreId,
                skus,
              ...
              for (const v of cachedVariants) {
                if (v.sku && v.price) {
                  // Normalize SKU for lookup (trim, uppercase)
                  const normalizedSku = v.sku.trim().toUpperCase();
                  shopifyPriceMap.set(normalizedSku, v.price);
                }
              }
              shopifyPricesLoaded = shopifyPriceMap.size;
              console.log(
                `[Import] Loaded ${shopifyPricesLoaded} Shopify prices for compare-at`,
              );
            }
          } catch (err) {
            console.error(
              "[Import] Error loading Shopify prices for compare-at:",
              err,
            );
          }
        }
        ...
        if (isSaleFile && shopifyPricesLoaded > 0) {
          ...
          console.log(
            `[Import] Applied sale pricing: ${priceMultiplier}x multiplier, ${shopifyPricesLoaded} compare-at prices set`,
          );
        }
        ...
        if (stockInfoRule) {
          console.log(
            `[Import] Calculating stockInfo for ${itemsToImport.length} items using rule: "${stockInfoRule.name}"`,
          );
          itemsToImport = itemsToImport.map((item: any) => ({
            ...item,
            stockInfo: calculateItemStockInfo(item, stockInfoRule),
        ...
        if (itemsToImport.length > 0) {
          if (updateStrategy === "full_sync") {
            ...
            importedCount = result.created;
            console.log(
              `[Import Full Sync] ${dataSource.name}: Atomic replace complete - deleted ${result.deleted}, created ${result.created} items`,
            );
          } else {
            // Replace (Create & Update): Only add/update items, keep items not in file
            console.log(
              `[Import Upsert] ${dataSource.name}: Upserting ${itemsToImport.length} items (existing items NOT deleted)`,
            );
            // Reset sale flags for regular inventory sources to clear stale consolidation state
            const isRegularInventory =
              (dataSource as any).sourceType !== "sales";
            const result = await storage.upsertInventoryItems(
              itemsToImport,
              dataSourceId,
              { resetSaleFlags: isRegularInventory },
          ...
          // SAFETY NET: Block empty file from deleting all data
          const existingCount =
            await storage.getInventoryItemCountByDataSource(dataSourceId);
          if (existingCount > 0) {
            console.error(
              `[Import] SAFETY BLOCK: File has 0 items but data source "${dataSource.name}" has ${existingCount} existing items. ` +
                `Blocking import to prevent data loss.`,
            );
            return res.status(400).json({
              error: "Import blocked - file contains no data",
              safetyBlock: true,
              message:
                `SAFETY NET: File has 0 items but would delete ${existingCount} existing items. ` +
                `This appears to be a corrupted or empty file. Import blocked to protect your data.`,
            });
          }
          // No existing data - safe to proceed (nothing to delete anyway)
          console.log(
            `[Import Full Sync] ${dataSource.name}: No items to import and no existing items - nothing to do`,
          );
        }
        ...
        // NOTE: We no longer consolidate at import time
        // Sale file merge now happens at SYNC time in shopify.ts
        // This prevents stale sale items from overwriting new regular imports
        let consolidationResult = null;
        let saleStylesRegistered = 0;

        if (isSaleFile) {
          // For sale files, register all styles in discontinued_styles table
          // This marks them as "owned" by this sale file
          try {
            ...
            console.log(
              `[Import] Registered ${saleStylesRegistered} styles from sale file "${dataSource.name}" (${styleResult.added} new, ${styleResult.updated} updated)`,
            );
          } catch (err) {
            console.error("[Import] Error registering sale file styles:", err);
          }

          // Also clean up old items from linked regular sources
          try {
            const cleanedUp =
              await cleanupSaleOwnedItemsFromRegular(dataSourceId);
            if (cleanedUp > 0) {
              console.log(
                `[Import] Cleaned up ${cleanedUp} old sale-owned items from regular sources`,
              );
            }
          } catch (err) {
        ...
        // Trigger background comparison job to detect mismatches for incremental sync
        // Get the first active Shopify store to run comparison against
        try {
          const stores = await storage.getShopifyStores();
          if (stores.length > 0) {
            const storeId = stores[0].id;
            console.log(
              `[Import] Starting background comparison job for store ${storeId}`,
            );
        ...
        if (discontinuedStylesFiltered > 0)
          processSummary += ` (${discontinuedStylesFiltered} discontinued styles skipped)`;
        if (discontinuedItemsRemoved > 0)
          processSummary += ` (${discontinuedItemsRemoved} discontinued items removed)`;
        if (saleStylesRegistered > 0)
          processSummary += ` (${saleStylesRegistered} sale styles registered)`;
        if (consolidationResult)
          processSummary += ` (consolidated: ${consolidationResult.saleVariantsAdded} sale, ${consolidationResult.regularVariantsDeleted} replaced)`;

        const strategyLabel =
          updateStrategy === "full_sync" ? "Full Sync" : "Create & Update";
        const importMessage =
        ...
        res.status(201).json({
          ...
          addedItems:
            updateStrategy === "replace" ? addedCount : ruleResult.addedCount,
          ...
          updateStrategy,
          consolidationResult,
          message: `Uploaded ${file.originalname} (${strategyLabel}) - ${importMessage}${processSummary}`,
        });
      } catch (error: any) {
        console.error("Error uploading file:", error);
  ...
  app.post("/api/data-sources/:id/combine-import", async (req, res) => {
    try {
      ...
      if (stagedFiles.length === 0) {
        return res.status(400).json({ error: "No staged files to combine" });
      }
      ...
      const isJovaniSaleFormat =
        cleaningConfig?.pivotedFormat?.vendor === "jovani";

      // Check if this is a sale file (needed for saleOwnsStyle flag during item collection)
      const isSaleFile = (dataSource as any).sourceType === "sales";
      ...
      for (const file of stagedFiles) {
        ...
        // - Style row: STYLE has value, COLOR is empty
        // - Misaligned style row: STYLE empty, COLOR is purely numeric (e.g., "1012")
        // - Variant row: COLOR has non-numeric value, SIZE has value
        ...
        for (const row of rows) {
          ...
          // For pre-parsed pivoted format, use direct column names
          // Otherwise use column mapping
          let sku: string;
          let style: string;
          let size: string;
          let color: string;
          ...
          if (isJovaniSaleFormat && !isPivotedPreParsed) {
            ...
            // Helper: Check if value is a style number (4-6 digits, optionally with # prefix)
            const isStyleNumber = (val: string) => /^#?\d{4,6}$/.test(val);

            // Helper: Check if value is a valid color (at least 2 letters - allows digits like "Back White 2")
            const isValidColor = (val: string) => /[a-zA-Z]{2,}/.test(val);

            // Check if this is a style header row:
            // 1. STYLE has value and COLOR is empty
            // 2. OR STYLE is empty but COLOR is purely numeric (misaligned style)
            ...
            if (isStyleRowNormal || isStyleRowNumeric) {
              ...
              console.log(
                `[Jovani Combine] Found style: ${jovaniCurrentStyle}`,
              );
              continue; // Skip style header rows
            }

            if (isStyleRowMisaligned) {
              jovaniCurrentStyle = rawColor; // The numeric value in COLOR is actually the style
              console.log(
            ...
            // Skip rows without a current style context
            if (!jovaniCurrentStyle) {
              continue;
            }

            // Skip rows with no color, numeric-only color, or invalid color (must have 2+ letters)
            if (
              !rawColor ||
              isPurelyNumeric(rawColor) ||
              !isValidColor(rawColor)
            ) {
              continue;
            }

            // This is a valid variant row - use the current style context
          ...
          if (cleaningConfig.combinedVariantColumn) {
            ...
            if (parts.length >= 3) {
              ...
              size = parts[parts.length - 1] || "";
              color = parts[parts.length - 2] || "";
              // Join remaining parts as style (handles styles with dashes)
              style = parts.slice(0, parts.length - 2).join(delimiter) || "";
            } else if (parts.length === 2) {
              // Two parts: assume style-size (no color)
              style = parts[0] || "";
          ...
          // Parse future date value (similar to shipDate parsing)
          ...
          allItems.push({
            ...
            futureStock,
            futureDate,
            // CRITICAL: Set hasFutureStock flag for items with ship dates or future dates
            // This ensures size expansion works even with 0 stock
      ...
      // === FUTURE STOCK ZEROING FOR COMBINE STEP ===
      // Detect if combined items have varying ship dates (real ship dates vs snapshot date)
      // If items have MULTIPLE dates, they're real ship dates (e.g., "inbound" file) — zero stock for future-dated items
      // If all items share ONE date, it's a snapshot/metadata date (e.g., "available" file) — keep stock
      ...
      if (uniqueCombineDates.size > 1) {
        ...
        // Use stockInfoConfig.dateOffsetDays for offset (e.g., -5 for Mon Cheri)
        ...
        for (const item of allItems) {
          ...
          if (mdyMatch) {
            ...
            // Try ISO or other parseable formats (YYYY-MM-DD)
          ...
          if (
            ...
            // Future date (beyond offset window) — zero out stock, preserve as pre-order
            ...
            // Within offset window — keep stock (vendor date is padded, item is actually available)
            withinOffsetKept++;
          }
        }
        ...
        if (futureZeroed > 0 || withinOffsetKept > 0) {
          console.log(
            `[Routes Combine] Future stock zeroing: ${futureZeroed} items zeroed, ${withinOffsetKept} kept (within ${offset}-day offset). ${uniqueCombineDates.size} unique dates.`,
          );
        }
      } else if (uniqueCombineDates.size === 1) {
        console.log(
          `[Routes Combine] Single date detected (${[...uniqueCombineDates][0]}) — snapshot mode, preserving stock`,
        );
      }
      ...
      if (
        ...
        if (shopifyStoreId) {
          console.log(
            `[Combine] Applying price-based size expansion for data source "${dataSource.name}"`,
          );
          try {
            const cacheVariants =
              await storage.getVariantCacheProductStyles(shopifyStoreId);
            const stylePriceMap = buildStylePriceMapFromCache(cacheVariants);
            console.log(
              `[Combine] Built style price map with ${stylePriceMap.size} styles`,
            );
            ...
            if (priceBasedExpansionCount > 0) {
              console.log(
                `[Combine] Price-based expansion added ${priceBasedExpansionCount} size variants`,
              );
            }
          } catch (expansionError) {
            console.error(
              `[Combine] Price-based expansion error:`,
              expansionError,
            );
          }
        }
      ...
      // Step 3: Handle discontinued styles based on source type
      // Note: isSaleFile already defined above for saleOwnsStyle flag
      const linkedSaleDataSourceId = (dataSource as any)
        .assignedSaleDataSourceId;
      let discontinuedStylesFiltered = 0;
      let discontinuedItemsRemoved = 0;
      ...
      if (!isSaleFile && linkedSaleDataSourceId) {
        // Regular file with linked sale file: Filter out discontinued styles and remove existing items
        console.log(
          `[Combine] Regular file "${dataSource.name}" - checking for discontinued styles from sale file`,
        );
        ...
        if (discontinuedItemsRemoved > 0) {
          console.log(
            `[Combine] Removed ${discontinuedItemsRemoved} existing inventory items with discontinued styles`,
          );
        }
        ...
        if (discontinuedStylesFiltered > 0) {
          console.log(
            `[Combine] Filtered out ${discontinuedStylesFiltered} items with discontinued styles`,
          );
        }
      }
      ...
      if (shopifyStoreIdForCompareAt && useCompareAtPrice && isSaleFile) {
        try {
          ...
          if (skus.length > 0) {
            console.log(
              `[Combine] Looking up Shopify prices for ${skus.length} SKUs (sale file compare-at)...`,
            );
            const cachedVariants = await storage.getVariantCacheBySKUs(
              shopifyStoreIdForCompareAt,
              skus,
            ...
            shopifyPricesLoaded = shopifyPriceMap.size;
            console.log(
              `[Combine] Loaded ${shopifyPricesLoaded} Shopify prices for compare-at`,
            );
          }
        } catch (err) {
          console.error(
            "[Combine] Error loading Shopify prices for compare-at:",
            err,
          );
        }
      }
      ...
      if (isSaleFile && shopifyPricesLoaded > 0) {
        ...
        console.log(
          `[Combine] Applied sale pricing: ${priceMultiplierForSale}x multiplier, ${shopifyPricesLoaded} compare-at prices set`,
        );
      }
      ...
      if (stockInfoRuleCombine) {
        console.log(
          `[Combine] Calculating stockInfo for ${itemsToImport.length} items using rule: "${stockInfoRuleCombine.name}"`,
        );
        itemsToImport = itemsToImport.map((item: any) => ({
          ...item,
          stockInfo: calculateItemStockInfo(item, stockInfoRuleCombine),
        }));
      }
      ...
      if (itemsToImport.length > 0) {
        if (updateStrategy === "full_sync") {
          ...
          importedCount = result.created;
          console.log(
            `[Combine Full Sync] ${dataSource.name}: Atomic replace complete - deleted ${result.deleted}, created ${result.created} items`,
          );
        } else {
          // Replace (Create & Update): Only add/update items, keep items not in file
          console.log(
            `[Combine Upsert] ${dataSource.name}: Upserting ${itemsToImport.length} items (existing items NOT deleted)`,
          );
          // Reset sale flags for regular inventory sources to clear stale consolidation state
          const isRegularInventory = (dataSource as any).sourceType !== "sales";
          const result = await storage.upsertInventoryItems(
        ...
        // SAFETY NET: Block empty combine from deleting all data
        const existingCount =
          await storage.getInventoryItemCountByDataSource(dataSourceId);
        if (existingCount > 0) {
          console.error(
            `[Combine] SAFETY BLOCK: Combined files have 0 items but data source "${dataSource.name}" has ${existingCount} existing items. ` +
              `Blocking import to prevent data loss.`,
          );
          return res.status(400).json({
            error: "Import blocked - combined files contain no data",
            safetyBlock: true,
            message:
              `SAFETY NET: Combined files have 0 items but would delete ${existingCount} existing items. ` +
              `This appears to be a corrupted or empty file. Import blocked to protect your data.`,
          });
        }
        // No existing data - safe to proceed (nothing to delete anyway)
        console.log(
          `[Combine Full Sync] ${dataSource.name}: No items to import and no existing items - nothing to do`,
        );
      }

      // For sale files, register styles in discontinued_styles table
      if (isSaleFile && itemsToImport.length > 0) {
        try {
          ...
          saleStylesRegistered = styleResult.total;
          console.log(
            `[Combine] Registered ${saleStylesRegistered} styles from sale file "${dataSource.name}"`,
          );
        } catch (err) {
          console.error("[Combine] Error registering sale file styles:", err);
        }
      }
      ...
      if (ruleResult.addedCount > 0)
        processSummary += ` (+${ruleResult.addedCount} expanded)`;
      if (discontinuedStylesFiltered > 0)
        processSummary += ` (${discontinuedStylesFiltered} discontinued skipped)`;
      if (saleStylesRegistered > 0)
        processSummary += ` (${saleStylesRegistered} sale styles registered)`;
      ...
      try {
        ...
        // Don't fail the import if consolidation fails
      }
      ...
      res.json({
        ...
        filteredItems: ruleResult.filteredCount,
        updateStrategy,
        message: `Combined ${stagedFiles.length} files (${strategyLabel}) - ${importMessage}${processSummary}`,
      });
    } catch (error: any) {
  ...
  app.post("/api/data-sources/:id/fetch-url", async (req, res) => {
    try {
      ...
      if (items.length === 0) {
        return res
          .status(400)
          .json({ error: "File contains no valid data rows" });
      }

      // Helper function to get prefix for a style based on custom rules
      // For sale files, strips "Sale" or "Sales" from the prefix to match regular file naming
      const getStylePrefix = (style: string): string => {
        if (
          cleaningConfig?.useCustomPrefixes &&
      ...
      // Step 0: Apply skip rule filtering (matching email/manual import behavior)
      // Items with shouldSkip=true are filtered out, unless skipUnlessContinueSelling is true AND continueSelling is enabled
      const continueSelling = (dataSource as any).continueSelling ?? true; // Default to true
      ...
      if (filteredItems.length < items.length) {
        console.log(
          `[URL Import] Step 0: Filtered out ${items.length - filteredItems.length} items based on skip rule`,
        );
      }

      // Step 0.5: Filter out discontinued items that have ZERO stock (matching email/manual import behavior)
      // "D" means discontinued - but if there's still stock, we should import it to sell remaining inventory
      // Only filter out: discontinued === true AND stock === 0 AND no future stock flags
      // This removes truly dead inventory while preserving pre-orders
      const discontinuedZeroStock = filteredItems.filter(
      ...
      if (discontinuedZeroStock.length > 0) {
        ...
        if (actuallyFiltered > 0) {
          console.log(
            `[URL Import] Step 0.5: Filtered out ${actuallyFiltered} discontinued (D) items with zero stock (preserved ${discontinuedZeroStock.length - actuallyFiltered} with future stock)`,
          );
        }
      }
      ...
      if (
        ...
        if (shopifyStoreId) {
          console.log(
            `[Email Import] Applying price-based size expansion for data source "${dataSource.name}"`,
          );
          try {
            const cacheVariants =
              await storage.getVariantCacheProductStyles(shopifyStoreId);
      ...
      if (shopifyStoreIdForCompareAt && useCompareAtPrice && isSaleFile) {
        try {
          ...
          if (skus.length > 0) {
            console.log(
              `[URL Import] Looking up Shopify prices for ${skus.length} SKUs (sale file compare-at)...`,
            );
            const cachedVariants = await storage.getVariantCacheBySKUs(
              shopifyStoreIdForCompareAt,
              skus,
            ...
            shopifyPricesLoaded = shopifyPriceMap.size;
            console.log(
              `[URL Import] Loaded ${shopifyPricesLoaded} Shopify prices for compare-at`,
            );
          }
        } catch (err) {
          console.error(
            "[URL Import] Error loading Shopify prices for compare-at:",
            err,
          );
        }
      }
      ...
      if (isSaleFile && shopifyPricesLoaded > 0) {
        ...
        console.log(
          `[URL Import] Applied sale pricing: ${priceMultiplierForSale}x multiplier, ${shopifyPricesLoaded} compare-at prices set`,
        );
      }
      ...
      if (stockInfoRuleUrl) {
        console.log(
          `[URL Import] Calculating stockInfo for ${itemsAfterExpansion.length} items using rule: "${stockInfoRuleUrl.name}"`,
        );
        itemsAfterExpansion = itemsAfterExpansion.map((item: any) => ({
          ...item,
          stockInfo: calculateItemStockInfo(item, stockInfoRuleUrl),
        }));
      }

      // ============================================================
      // STEP 4: HANDLE DISCONTINUED STYLES (matching manual upload behavior)
      // ============================================================
      const linkedSaleDataSourceId = (dataSource as any)
        .assignedSaleDataSourceId;
      let discontinuedStylesFiltered = 0;
      ...
      // Get the final items to import
      let itemsToImport = itemsAfterExpansion;

      if (!isSaleFile && linkedSaleDataSourceId) {
        // Regular file with linked sale file: Filter out discontinued styles and remove existing items
        console.log(
          `[URL Import] Regular file "${dataSource.name}" - checking for discontinued styles from sale file`,
        );
        ...
        if (discontinuedItemsRemoved > 0) {
          console.log(
            `[URL Import] Removed ${discontinuedItemsRemoved} existing items with discontinued styles`,
          );
        }
        ...
        if (discontinuedStylesFiltered > 0) {
          console.log(
            `[URL Import] Filtered out ${discontinuedStylesFiltered} items with ${filterResult.discontinuedStyles.length} discontinued styles: ${filterResult.discontinuedStyles.slice(0, 3).join(", ")}${filterResult.discontinuedStyles.length > 3 ? "..." : ""}`,
          );
        }
      }

      // Check update strategy (matching manual upload behavior)
      const updateStrategy = (dataSource as any).updateStrategy || "full_sync";
      let importedCount = 0;
      let addedCount = 0;
      let updatedCount = 0;
      ...
      if (itemsToImport.length > 0) {
        if (updateStrategy === "full_sync") {
          ...
          console.log(
            `[URL Import Upsert] ${dataSource.name}: Upserting ${itemsToImport.length} items (existing items NOT deleted)`,
          );
          const isRegularInventory = (dataSource as any).sourceType !== "sales";
          const result = await storage.upsertInventoryItems(
            itemsToImport,
        ...
        // Step 4.5: For sale files, register styles in discontinued_styles table
        if (isSaleFile) {
          try {
            const styleResult = await registerSaleFileStyles(
              dataSourceId,
            ...
            saleStylesRegistered = styleResult.total;
            console.log(
              `[URL Import] Registered ${saleStylesRegistered} styles from sale file "${dataSource.name}"`,
            );
          } catch (err) {
            console.error(
              "[URL Import] Error registering sale file styles:",
              err,
            );
          }
        }
        ...
        // SAFETY NET: Block empty file from deleting all data
        // Check if there's existing data that would be deleted
        const existingCount =
          await storage.getInventoryItemCountByDataSource(dataSourceId);
        if (existingCount > 0) {
          console.error(
        ...
        // No existing data - safe to proceed (nothing to delete anyway)
        console.log(
          `[URL Import Full Sync] ${dataSource.name}: No items to import and no existing items - nothing to do`,
        );
      }
      ...
      await storage.createUploadedFile({
        dataSourceId,
        fileName: url.split("/").pop() || "url-import.xlsx",
        fileSize: buffer.length,
        fileStatus: "imported",
        rowCount: ruleResult.items.length,
        headers,
