...
export async function processEmailAttachment(
  ...
  try {
    ...
    const eLog = (msg: string) => {
      ...
      try {
        fs.appendFileSync("/tmp/email_download.log", line);
      } catch {}
    };

    eLog(
      `[EmailImport] === Processing file: "${filename}" for "${dataSource.name}" ===`,
    );
    eLog(
      `[EmailImport] isMultiFile=${isMultiFile}, forceStage=${forceStage}, ingestionMode=${(dataSource as any).ingestionMode}`,
    );
    ...
    eLog(
      `[EmailImport] Buffer preview (first 200 chars): ${buffer.toString("utf-8", 0, Math.min(200, buffer.length))}`,
    );
    ...
    if (!detectedFormat) {
      try {
        ...
        if (rawData.length > 0) {
          ...
          if (detectedFormat) {
            eLog(
              `[EmailImport] AUTO-DETECTED format: "${detectedFormat}" using shared detector`,
            );
        ...
        eLog(
          `[EmailImport] Format auto-detection error (non-fatal): ${detectErr.message}`,
        );
      }
    }

    eLog(
      `[EmailImport] detectedFormat="${detectedFormat}", pivotConfig?.format="${pivotConfig?.format}", pivotConfig?.enabled=${pivotConfig?.enabled}`,
    );
    ...
    if (detectedFormat) {
      ...
      console.log(
        `[Email Import] Shared parser (${detectedFormat}) result: ${items.length} items for "${dataSource.name}"`,
      );
    ...
    if (cleaningConfig && items.length > 0) {
      ...
      if (hasAnyCleaning) {
        console.log(
          `[Email Import] Applying data source cleaning rules to ${items.length} items`,
        );
    ...
    eLog(
      `[EmailImport] Parse result: headers=${JSON.stringify(headers?.slice(0, 15))}`,
    );
    eLog(
      `[EmailImport] Parse result: rows=${rows?.length || 0}, items=${items?.length || 0}`,
    );
    if (rows?.length > 0) {
      eLog(
        `[EmailImport] Sample row[0]: ${JSON.stringify(rows[0]?.slice(0, 10))}`,
      );
    }
    if (rows?.length > 1) {
      eLog(
        `[EmailImport] Sample row[1]: ${JSON.stringify(rows[1]?.slice(0, 10))}`,
      );
    }
    if (items?.length > 0) {
    ...
    if (validationConfig.enabled !== false) {
      ...
      // DEFAULT SAFETY CHECK: Block if row count drops by more than the configured threshold
      ...
      if (
        ...
        const dropPercent = actualRowCount <= 0
          ? 100
          : Math.round(((lastRowCount - actualRowCount) / lastRowCount) * 100);
        if (dropPercent > safetyThreshold) {
          const errorMessage = `SAFETY BLOCK: Item count dropped ${dropPercent}% (from ${lastRowCount} to ${actualRowCount}). Threshold is ${safetyThreshold}%. Import blocked to protect your data.`;
          await logSystemError(
            dataSourceId,
            "row_count_error",
            errorMessage,
    ...
    if (isMultiFile) {
      ...
      if (rows?.length > 0) {
        ...
        eLog(
          `[EmailImport] Staging row[last]: ${JSON.stringify(rows[rows.length - 1])}`,
        );
      }
    ...
    if (itemsToImport.length > 0) {
      if (updateStrategy === "full_sync") {
        ...
        const emailSafetyThreshold = (dataSource as any).safetyThreshold ?? 50;
        if (emailSafetyThreshold > 0 && existingCount > 0) {
          const dropPct = itemsToImport.length <= 0 ? 100 : ((existingCount - itemsToImport.length) / existingCount) * 100;
          if (dropPct > emailSafetyThreshold) {
            const errMsg = `SAFETY BLOCK: Item count dropped ${Math.round(dropPct)}% (from ${existingCount} to ${itemsToImport.length}). Threshold is ${emailSafetyThreshold}%. Import blocked to protect your data.`;
            console.error(`[Email Import] ${errMsg}`);
            await logSystemError(
              dataSourceId,
              "row_count_error",
              errMsg,
              { previousRowCount: existingCount, actualRowCount: itemsToImport.length, safetyBlock: true, threshold: emailSafetyThreshold },
              "critical",
            );
            try {
              await sendImmediateImportAlert({
                dataSourceName: dataSource.name,
                filename,
                errorType: "row_count_error",
                errorMessage: errMsg,
                details: { existingCount, newCount: itemsToImport.length, threshold: emailSafetyThreshold },
              });
            } catch (alertErr: any) {
              console.error(`[Email Import] Failed to send safety alert: ${alertErr.message}`);
            }
            failImport(dataSourceId, errMsg);
            return { success: false, rowCount: 0, error: errMsg };
          }
        }
        console.log(
  ...
  try {
    ...
    if (!columnMapping || Object.keys(columnMapping).length === 0) {
      ...
      if (Object.keys(autoMapping).length > 0) {
        ...
        cLog(
          `[Combine] columnMapping was empty — auto-detected from headers: ${JSON.stringify(columnMapping)}`,
        );
      }
    }
    ...
    cLog(
      `[Combine] isJovaniSaleFormat: ${isJovaniSaleFormat}, isSaleFile: ${isSaleFile}`,
    );
    ...
    for (const file of stagedFiles) {
      ...
      cLog(
        `[Combine] File: ${(file as any).fileName || "unknown"}, headers: ${JSON.stringify(headers?.slice(0, 10))}, rows: ${rows.length}`,
      );
    ...
    cLog(
      `[Combine] After cleanInventoryData: ${cleanResult.items?.length || 0} items`,
    );
    ...
    cLog(
      `[Combine] After applyImportRules: ${importRulesResult.items?.length || 0} items`,
    );
    ...
    cLog(
      `[Combine] After applyVariantRules: ${ruleResult.items?.length || 0} items`,
    );
    if (ruleResult.items?.length > 0) {
      cLog(
        `[Combine] variantRules sample[0]: ${JSON.stringify(ruleResult.items[0])}`,
      );
    }
    ...
    if (
      ...
      if (shopifyStoreId) {
        ...
        try {
          ...
          cLog(
            `[Combine] Price-based expansion error: ${expansionError.message}`,
          );
        }
      }
    }
    ...
    if (shopifyStoreIdForCompareAt && useCompareAtPrice && isSaleFile) {
      try {
        ...
        cLog(
          `[Combine] Error loading Shopify prices for compare-at: ${err.message}`,
        );
      }
    }
    ...
    if (itemsToImport.length > 0) {
      if (updateStrategy === "full_sync") {
        const existingCount = await storage.getInventoryItemCountByDataSource(dataSourceId);
        const combineSafetyThreshold = (dataSource as any).safetyThreshold ?? 50;
        if (combineSafetyThreshold > 0 && existingCount > 0) {
          const dropPct = itemsToImport.length <= 0 ? 100 : ((existingCount - itemsToImport.length) / existingCount) * 100;
          if (dropPct > combineSafetyThreshold) {
            const errMsg = `SAFETY BLOCK: Item count dropped ${Math.round(dropPct)}% (from ${existingCount} to ${itemsToImport.length}). Threshold is ${combineSafetyThreshold}%. Import blocked to protect your data.`;
            cLog(errMsg);
            await logSystemError(
              dataSourceId,
              "row_count_error",
              errMsg,
              { previousRowCount: existingCount, actualRowCount: itemsToImport.length, safetyBlock: true, threshold: combineSafetyThreshold },
              "critical",
            );
            failImport(dataSourceId, errMsg);
        ...
        cLog(
          `[Combine Upsert] ${dataSource.name}: Added ${result.added}, updated ${result.updated} items`,
        );
      }
    } else if (updateStrategy === "full_sync") {
      ...
      if (existingCount > 0) {
        cLog(
          `[Combine] SAFETY BLOCK: 0 items but ${existingCount} existing — NOT deleting`,
        );
        // Don't delete, just warn
      } else {
        cLog(
          `[Combine] No items to import and no existing items — nothing to do`,
        );
      }
    }
    ...
    cLog(
      `[Combine] Complete: ${importedCount} items imported for "${dataSource.name}"`,
    );
...
export function parsePRDateHeaderFormat(
  ...
  console.log(
    `[PRDateHeaders] Raw headers (first 20): ${headers.slice(0, 20).join(" | ")}`,
  );
